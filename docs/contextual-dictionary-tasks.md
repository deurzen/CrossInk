...
| ID | Status | Unit of work | Completion gate |
| --- | --- | --- | --- |
| C24 | Done | Key learning state and book artifacts exclusively by canonical UUID | Identity mismatches fail before lookup or state access |
...
## Phase F — validation and production qualification
...
The contextual path is the sole implementation. Production release still
requires C33–C36 qualification, newly compiled v4 EPUBs, and recoverable source
installation; no migration path is provided for removed formats.
...
The frozen version-4 `language.bin` header treats bytes `[16,32)` as the
canonical identity, enforces analyzer version 1 and target `und`, and accepts
the contextual provenance flag bits. `Epub` exposes that canonical identity.
Lookup selects `/.crosspoint/lexicons/<uuid>/`, validates metadata and lexeme
bounds, and keys the WAL state store by the same UUID. No other artifact version
or package root is accepted.
...
A missing or mismatched canonical UUID must fail lookup before opening learning
state or a definition source.
...
`DefinitionPager` uses a small callback-backed entry reader. Lookup retains one `entries.bin` path
...
The identity-count policy is allocation-free and shared by filtering and status
updates, preventing the two paths from drifting. On X3/X4, mark an ambiguous item Known,
...
Firmware distinguishes a missing canonical lexicon from an incompatible one.
Both show separate translated installation/reinstallation messages before the
shortlist opens.
