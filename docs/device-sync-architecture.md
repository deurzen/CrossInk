...
- Phase 2 groundwork defines stable MAC-bound identity, crash-safe pair records, deterministic roles, replay counters, strong-entropy nonces, RFC-vector-tested X25519/HKDF pairing keys, and bounded category/path policy.
...
The pinned ESP32-C3 framework compile probe confirms mbedTLS Curve25519, ECDH, HKDF, and SHA-256 support. The implementation matches RFC 7748 X25519 and RFC 5869 HKDF-SHA256 host vectors, derives transcript-bound pair/confirmation/directional session keys, and clears private/shared/derived secret buffers. Canonical transcript construction, confirmation messages, and committing the pair record remain pending. Runtime heap peak, stack high-water, and timing must still be measured on X3/X4 before finalizing this choice; mbedTLS internally allocates multiprecision state during the one-shot exchange.
